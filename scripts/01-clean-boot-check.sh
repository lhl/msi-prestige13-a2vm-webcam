...
focus_journal=$(journalctl -b -k --no-pager | rg \
  'TPS68470 REVID|Found supported sensor|Connected 1 cameras|applying extra post-power-on delay|applying gpio release sequence|chip id read attempt|chip id attempt|sensor identified on attempt|Failed to enable|failed to power on|failed to find sensor|probe with driver ov5675 failed' \
  || true)
